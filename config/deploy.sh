...
if check_diff "config/ilia.service"; then
    echo "$EG update ilia service"
    cp config/ilia.service /etc/systemd/system/ --force
    systemctl daemon-reload
    echo $SPACER
fi

echo "$EG restart ilia service"
systemctl restart ilia
echo $SPACER
